...
# Load the YAML file
with open('api_keys_bot_new.yaml', 'r') as file:
    api_keys = yaml.safe_load(file)

# Set API Keys and Authentification
logging.info('Setting NewsAPI API Key')
NEWSAPI_API_KEY = api_keys['NEWSAPI_API_KEY'] # replace with own API key

logging.info('Setting OpenAI API Key')
openai.api_key = api_keys['openai.api_key'] # replace with own API key

logging.info('Setting Twitter API Key')
client= tweepy.Client(api_keys['bearer_token'], api_keys['twitter_api_key'], api_keys['twitter_api_secret'], api_keys['twitter_access_token'], api_keys['twitter_access__secret'])
auth=tweepy.OAuth1UserHandler(api_keys['twitter_api_key'], api_keys['twitter_api_secret'],
                              api_keys['twitter_access_token'], api_keys['twitter_access__secret'])
auth.set_access_token(api_keys['twitter_access_token'],
                      api_keys['twitter_access__secret'])
...
def fetch_news(number=10):
    ...
    url = f"https://newsapi.org/v2/everything?q=haryana&sortBy=publishedAt&apiKey={NEWSAPI_API_KEY}"
...
def main_bot():
    ...
    relevant_topics ="[haryana, Indian politics, election, Indian government, general election, BJP, Congress, Lok Sabha Elections, politics, political party, citizens, voters, modi, rahul gandhi]"
